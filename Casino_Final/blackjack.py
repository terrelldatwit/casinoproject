...
class Blackjack:
    ...
    def start_round(self):
        ...
        self.player_total.set(str(p_score))
        self.status_var.set(f"Player Hand 1: {p_score}   Dealer: ?")
    ...
    def hit(self):
        ...
        if self.current_hand == 1:
            ...
            self.player_total.set(f"Hand 1: {p}")
            if p > 21:
                self.current_hand = 2
                self.status_var.set(f"Hand 1 busts. Playing Hand 2. Score: {calculate_score(self.second_hand)}")
                self.play_next_hand()
            ...
            self.player_total.set(f"Hand 2: {p}")
            if p > 21:
                self.end_round()
    ...
    def stand(self):
        ...
        if self.is_split and self.current_hand == 1:
            self.current_hand = 2
            self.status_var.set(f"Hand 1 stands. Playing Hand 2. Score: {calculate_score(self.second_hand)}")
            self.play_next_hand()
        else:
            self.end_round()
    ...
    def split_hand(self):
        if len(self.player_hand) == 2 and self.player_hand[0] == self.player_hand[1] and self.balance >= self.bet:
            ...
            p_score = calculate_score(self.player_hand)
            self.player_total.set(f"Hand 1: {p_score}")
            self.status_var.set(f"Split! Playing Hand 1. Score: {p_score}")
            self.split_btn.config(state="disabled")
    ...
    def play_next_hand(self):
        self.current_hand = 2
        self.player_total.set(f"Hand 2: {calculate_score(self.second_hand)}")
        self.status_var.set(f"Playing Hand 2. Score: {calculate_score(self.second_hand)}")
    ...
    def plot_net_winnings(self):
        try:
            ...
            with sqlite3.connect(DB_PATH) as conn:
                ...
                # Calculate net winnings for each session (money_won - bet_amount)
                net_winnings = [row[1] - row[2] for row in data]
